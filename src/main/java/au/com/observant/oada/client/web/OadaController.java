...
public class OadaController {
    ...
    public String oadaEntryPoint() {
        ...
        // Once authorization is completed OADA OAuth2 server redirects browser back to this method and we will call the
        // actual endpoint in Observant OADA Resource server.
        Map<String, String> result = restTemplate.getForObject(url, Map.class);
        log.debug("Response from OADA resource server: {}", result);
        // Next line will redirect browser to actual page in this server once user is authenticated by OAauth2 server.
        return VIEW_OADA;
    }
}
